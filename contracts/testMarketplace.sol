...
contract testMarketplace is IRegistry, ERC721Holder, ERC1155Receiver, ERC1155Holder {
    ...
    uint256 public defaultRentFee = 2;
    mapping(address => uint256) public rentFees;
    ...
    function sell(
        ...
        bundleSell( handleSell, createSellCallData(
                nftStandard,
                nftAddress,
                tokenID,
                lendAmount,
    ...
    function handleSell(IRegistry.CallData memory cd, address _to) private {
        for (uint256 i = cd.left; i < cd.right; i++) {
            uint8 paymentTokenIx = uint8(cd.paymentToken[i]);
            ERC20 paymentERC20Token = ERC20(resolver.getPaymentToken(paymentTokenIx));
            uint256 finalAmt = cd.lendAmount[i];
            uint256 takenFee = 0;
            if (defaultRentFee != 0) {
                takenFee = takeFee(
                    finalAmt,
            ...
            paymentERC20Token.safeTransfer(_to, finalAmt - takenFee);
        }
        ...
        safeTransfer(
            ...
            _to,
            sliceArr(cd.tokenID, cd.left, cd.right, 0),
            sliceArr(cd.lendAmount, cd.left, cd.right, cd.left)
        );
    }
    ...
    function handleStopRent(IRegistry.CallData memory cd) private {
        for (uint256 i = cd.left; i < cd.right; i++) {
            ...
            distributePayments(lending, renting, secondsSinceRentStart, cd.nftAddress[i]);
    ...
    function handleClaimRent(CallData memory cd) private {
        for (uint256 i = cd.left; i < cd.right; i++) {
            ...
            distributeClaimPayment(lending, renting, cd.nftAddress[i]);
    ...
    function bundleSell(
        function(IRegistry.CallData memory, address) handler,
        IRegistry.CallData memory cd,
        address _to
    ) private {
        require(cd.nftAddress.length > 0, "ReNFT::no nfts");
    ...
    function takeFee(uint256 rentAmt, IResolver.PaymentToken paymentToken, address nftAddress)
    private
    returns (uint256 fee)
    {
        if (rentFees[nftAddress] == 0) {
            fee = rentAmt * defaultRentFee;
        }
        else {
            fee = rentAmt * rentFees[nftAddress];
        }

        fee /= 10000;
        uint8 paymentTokenIx = uint8(paymentToken);
        ERC20 pmtToken = ERC20(resolver.getPaymentToken(paymentTokenIx));
        pmtToken.safeTransfer(Ownable(nftAddress).owner(), fee);
    }
    ...
    function distributePayments(
        ...
        if (defaultRentFee != 0) {
            uint256 takenFee = takeFee(sendLenderAmt, lending.paymentToken, nftAddress);
    ...
    function setNFTGeneratorCommission(address nftAddress, uint256 commissionRate)
    public
    {
        require(nftAddress != address(0), 'Invalid address!');
        require(Ownable(nftAddress).owner() == msg.sender, 'Send should be NFT Contract owner!');
        require(commissionRate > 0, 'Invalid address!');
        rentFees[nftAddress] = commissionRate;
    }
}
